...
fixture `Vehicle Comparison Tool`
    .page `http://local.caranddriver.com/`;
...
test.skip('Global page props', async t => {
...
test.skip('Add a vehicles', async t => {
    let compareComponent = page.compareComponent;
    let addVehicleBtn = await Selector('#addVehicleBtn');
    console.log('1', addVehicleBtn);
    await t
        .expect(compareComponent.exists).ok()
        .expect(page.addVehicleCard.exists).ok()
        ...
        .click(addVehicleBtn)
    await t
        expect(addVehicleModal.exists).ok();
    await t
